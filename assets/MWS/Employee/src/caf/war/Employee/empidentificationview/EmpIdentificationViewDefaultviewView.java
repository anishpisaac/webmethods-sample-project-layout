 ...
 * 
 */
package caf.war.Employee.empidentificationview;
...
@ManagedBean(name = "EmpIdentificationViewDefaultviewView")
@SessionScoped
@ExpireWithPageFlow
@DTManagedBean(displayName = "EmpIdentificationView/default", beanType = BeanType.PAGE)
public class EmpIdentificationViewDefaultviewView extends com.webmethods.caf.faces.bean.task.BaseTaskDetailsPageBean {
	...
	private static final String[][] TASKDISPLAYPROVIDER_PROPERTY_BINDINGS = new String[][] {
		{"#{TaskDisplayProvider.taskID}", "#{EmpIdentificationView.taskID}"},
	};
	...
	public String cancelView() {
		try {
			...
			String url = getEmpIdentificationView().getFinishUrl();
	...
	public String completeTask() {
		try {
			if( !getEmpIdentification().isUpdateable() ){
			...
			// do the work
			getEmpIdentification().completeTask(); 
	...
	public String submitTask() {
		try {
			...
			// do the work
			getEmpIdentification().applyChanges();
		} catch (Exception e) {
			error(e);
			log(e);
		}
	...
	public String assignToPrincipals() {
		try {
			...
			TaskContentProvider tp = getEmpIdentification();
	...
	private static final String[][] INITIALIZE_PROPERTY_BINDINGS = new String[][] {
		{"#{EmpIdentification.reset}", null}
	};
	private transient caf.war.Employee.empidentificationview.EmpIdentificationView empIdentificationView = null;
	...
	private static final String[][] EMPIDENTIFICATION_PROPERTY_BINDINGS = new String[][] {
		{"#{EmpIdentification.taskID}", "#{EmpIdentificationView.taskID}"},
		{"#{EmpIdentification.autoAccept}", "false"},
		{"#{EmpIdentification.adhocRouting}", "false"},
	};
	...
	public caf.war.Employee.empidentificationview.EmpIdentificationView getEmpIdentificationView()  {
		if (empIdentificationView == null) {
		    empIdentificationView = (caf.war.Employee.empidentificationview.EmpIdentificationView)resolveExpression("#{EmpIdentificationView}");
		}
		return empIdentificationView;
	}
